...
def projection_removal_test_next_token_padded_only(
    ...
    *,
    ...
    def _remove_projection(hid: torch.Tensor, cache: dict):
        ...
        # Optionally preserve the original norm
        if preserve_norm:
            h_norm = h.norm(dim=1, keepdim=True)  # (B, 1)
            h_new_norm = h_new.norm(dim=1, keepdim=True)  # (B, 1)
            # Avoid division by zero
            scale = torch.where(h_new_norm > 1e-8, h_norm / h_new_norm, torch.ones_like(h_norm))
            h_new = h_new * scale
...
def projection_onto_test_next_token_padded_only(
    config,
    model: torch.nn.Module,
    sampler,
    final_task_vecs: torch.Tensor,
    *,
    ...
    """
    Project onto majority task subspace at padded token positions.
    
    For each hidden vector h at a padded position:
    1. Compute its projection onto the span of final_task_vecs
    2. Use that projection as the new hidden state (optionally preserving norm)
    3. Use the modified hidden state for next-token prediction
    ...
    Args:
        ...
        return_logits: If True, return logits and other detailed outputs
        verbose: If True, print sanity check information
        preserve_norm: If True, scale the projection to preserve the original norm
        
    Returns:
        Dictionary with metrics and results (same format as projection_removal_test_next_token_padded_only)
    """
    if not getattr(sampler, "pad", False):
        raise ValueError("This function requires padded coin sequences (sampler.pad must be True).")
    ...
    def _project_onto(hid: torch.Tensor, cache: dict):
        """
        Project onto majority task subspace from hidden states at padded position.
        ...
        # Project onto subspace: h_new = proj (instead of h - proj)
        h_new = proj
        ...
        # Sanity check: verify Q.T @ h_new is approximately equal to Q.T @ h
        # Since h_new is the projection, Q.T @ h_new should equal Q.T @ h
        QTh_new = Q_dtype.T @ h_new.T  # (rank, B) - projection of h_new onto Q subspace
        QTh_orig = Q_dtype.T @ h.T  # (rank, B) - projection of original h onto Q subspace
        QTh_diff = (QTh_new - QTh_orig).norm(dim=0)  # (B,) - difference per sample
        cache["QTh_diff_norm"] = QTh_diff.detach()
        cache["QTh_diff_norm_mean"] = float(QTh_diff.mean().item())
        cache["QTh_diff_norm_max"] = float(QTh_diff.max().item())
        
        # Also store QTh_new_norm for consistency with removal function
    ...
    def _run_intervened():
        ...
        def hook_fn(module, inp, out):
            # out can be Tensor or tuple(Tensor, ...)
            if torch.is_tensor(out):
                return _project_onto(out, cache)
            if isinstance(out, tuple) and len(out) > 0 and torch.is_tensor(out[0]):
                new0 = _project_onto(out[0], cache)
                return (new0,) + out[1:]
            raise RuntimeError(f"Unsupported hook output type: {type(out)}")
    ...
    results = {
        ...
        "QTh_new_norm_mean": cache.get("QTh_new_norm_mean", 0.0),
        "QTh_new_norm_max": cache.get("QTh_new_norm_max", 0.0),
        "QTh_diff_norm_mean": cache.get("QTh_diff_norm_mean", 0.0),
        "QTh_diff_norm_max": cache.get("QTh_diff_norm_max", 0.0),
    }
    ...
    if return_logits:
        results.update({
            ...
            "kl_per_example": kl.detach(),
            "QTh_new_norm": cache.get("QTh_new_norm", None),
            "QTh_diff_norm": cache.get("QTh_diff_norm", None),
        })
...
def projection_onto_coin_padded_only(
    exp_name: str,
    n_major: int | None = None,
    n_minor: int = 64,
    n_ood: int = 30,
    ...
    """
    Apply projection onto majority task subspace at all padded positions for coin task.
    
    For each padded position p_idx, projects hidden states onto the majority task subspace
    (defined by final_task_vecs) and measures the effect on next-token prediction.
    ...
    if not getattr(sampler, "pad", False):
        raise ValueError(
            "projection_onto_coin_padded_only requires padded coin sequences (sampler.pad must be True)."
        )
    ...
    # Separate tasks into major, minor, and OOD
    n_major_available = int(getattr(sampler, "n_major_tasks", 0))
    n_minor_total = int(getattr(sampler, "n_minor_tasks", 0))
    ...
    for p_idx in range(P):
        ...
        # Process major tasks
        for tid in major_task_ids:
            r = projection_onto_test_next_token_padded_only(
                config=config,
                model=model,
                sampler=sampler,
                final_task_vecs=final_task_vecs,
...
def projection_onto_test_next_token_linear_padded_only(
    config,
    model: torch.nn.Module,
    train_task,
    final_task_vecs: torch.Tensor,
    *,
    ...
    """
    Project onto majority task subspace at padded token positions for linear regression.
    ...
    def _project_onto(hid: torch.Tensor, cache: dict):
        ...
        # Sanity check: verify Q.T @ h_new is approximately equal to Q.T @ h
        QTh_new = Q_dtype.T @ h_new.T  # (rank, B)
        QTh_orig = Q_dtype.T @ h.T  # (rank, B)
        QTh_diff = (QTh_new - QTh_orig).norm(dim=0)  # (B,)
        cache["QTh_diff_norm"] = QTh_diff.detach()
        cache["QTh_diff_norm_mean"] = float(QTh_diff.mean().item())
        cache["QTh_diff_norm_max"] = float(QTh_diff.max().item())
        
        # Also store QTh_new_norm for consistency
        QTh_new_norm = QTh_new.norm(dim=0)  # (B,)
        cache["QTh_new_norm"] = QTh_new_norm.detach()
        cache["QTh_new_norm_mean"] = float(QTh_new_norm.mean().item())
        cache["QTh_new_norm_max"] = float(QTh_new_norm.max().item())
...
def projection_onto_linear_padded_only(
    exp_name: str,
    n_major: int | None = None,
    n_minor: int = 64,
    n_ood: int = 30,
    ...
    """
    Apply projection onto majority task subspace at all padded positions for linear regression task.
    ...
    if not hasattr(model, "pad") or model.pad != "mapsto":
        raise ValueError(
            "projection_onto_linear_padded_only requires model with pad='mapsto' "
            f"(got pad={getattr(model, 'pad', None)})."
        )
    ...
    try:
        ...
        for p_idx in range(P):
            ...
            # Process major tasks
            for tid in major_task_ids:
                r = projection_onto_test_next_token_linear_padded_only(
                    config=config,
                    model=model,
                    train_task=train_task,
                    final_task_vecs=final_task_vecs,
...
def projection_removal_latent_padded_only(
    ...
    # Separate tasks into major, minor, and OOD
    # CRITICAL: The sampler uses mode="major" with task=0,1,2 for major tasks,
    # and mode="minor" with task indices into the minor_trans_mat for minor/OOD tasks.
    # get_latent_sampler structure: minor_trans_mat = [OOD tasks (first n_ood), original minor tasks (next k_minor)]
    n_major_available = int(getattr(sampler, "n_major_tasks", 0))
    n_minor_total = int(getattr(sampler, "n_minor_tasks", 0))  # Total in minor pool (OOD + original minor)
    ...
    # Task specifications: (mode, task_id) tuples
    # Major tasks: use mode="major" with task indices 0 to n_major_use-1
    major_tasks = [("major", i) for i in range(n_major_use)]
    
    # OOD tasks: use mode="minor" with task indices 0 to n_ood-1 (first n_ood in minor pool)
    ood_tasks = [("minor", i) for i in range(n_ood)] if n_ood > 0 else []
    
    # Minor tasks: use mode="minor" with task indices n_ood to n_ood+k_minor-1 (remaining in minor pool)
    minor_tasks = [("minor", n_ood + i) for i in range(k_minor)] if k_minor > 0 else []
    
    all_tasks = major_tasks + ood_tasks + minor_tasks
    ...
    for p_idx in range(P):
        ...
        # Process major tasks
        for task_mode, task_id in major_tasks:
            r = projection_removal_test_next_token_padded_only(
                config=config,
                model=model,
                sampler=sampler,
        ...
        # Process minor tasks
        for task_mode, task_id in minor_tasks:
            r = projection_removal_test_next_token_padded_only(
                config=config,
                model=model,
                sampler=sampler,
        ...
        # Process OOD tasks
        for task_mode, task_id in ood_tasks:
            r = projection_removal_test_next_token_padded_only(
                config=config,
                model=model,
                sampler=sampler,
...
def projection_onto_latent_padded_only(
    exp_name: str,
    n_major: int | None = None,
    n_minor: int = 256,
    n_ood: int = 40,
    ...
    """
    Apply projection onto majority task subspace at all padded positions for latent Markov task.
    ...
    if not getattr(sampler, "pad", False):
        raise ValueError(
            "projection_onto_latent_padded_only requires padded latent sequences (sampler.pad must be True)."
        )
