"""Plotting functions for latent Markov probes."""
...
def plot_averaging_r2_latent(
    ...
    for li, l_num in enumerate(layers_idx):
        ...
        raw_pooled = torch.stack(cell_means_list, dim=0).mean(dim=0)
        grand_mean = raw_pooled.mean(dim=(0, 1))

        demeaned_list = []
        for cm in cell_means_list:
            mu_t = cm.mean(dim=(0, 1))
            demeaned_list.append(cm - mu_t)
        pooled = torch.stack(demeaned_list, dim=0).mean(dim=0)
