...
def plot_inject_task_vector_across_layers(
    ...
    fig_hm.suptitle("Task-ID Accuracy after Injection (latent)", fontsize=15, y=1.02)
    plt.tight_layout()
    if show:
        plt.show()
    else:
    ...
    fig_kl.suptitle("Task-Vector Injection: KL & Accuracy (latent)", fontsize=15, y=1.02)
    plt.tight_layout()
    if show:
        plt.show()
    else:
    ...
    ax_o2.set_ylabel(r"KL(output $\|$ P(·|s, injected))", fontsize=14)
    ax_o2.set_title("OOD: KL to Injected Task", fontsize=13)
    ax_o2.set_xticks(layers)
    ax_o2.legend(fontsize=11)
    ax_o2.grid(alpha=0.3)
    fig_ood.suptitle("Task-Vector Injection into OOD Sequences (latent)", fontsize=15, y=1.02)
    plt.tight_layout()
    if show:
        plt.show()
    else:
